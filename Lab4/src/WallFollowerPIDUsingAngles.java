...
public class WallFollowerPIDUsingAngles {
	...
	public static void main(String[] args) {
		...
		//TachoMotorPort left = MotorPort.A.open(TachoMotorPort.class); // right
		//TachoMotorPort right = MotorPort.B.open(TachoMotorPort.class); // left
		...
		// configure values
		final int desiredDistance = 25;
	     final int noWall = 100;
		...
		while (!Button.ESCAPE.isDown()) {
			...
			if (error > 0) {
				left.stop(true);
				right.stop(true);
				left.setSpeed(downPower);
				right.setSpeed(upPower);
